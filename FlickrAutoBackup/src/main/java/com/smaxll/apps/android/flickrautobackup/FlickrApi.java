...
//                                            for (String tag : photo.getMachineTag()) {
                                            for (Tag tag : photo.getTags()) {
                                                if (tag.getId().startsWith("file:sha1sig")) {
                                                    uploadedPhotos.put(tag.getId(), photo.getId());
...
//			}
	...
	public static boolean upload(Media image, String photosetId, String photoSetTitle, Folder folder) {
		...
		String photoId = null;
		String sha1tag = Utils.getSHA1tag(image);
		ConnectivityManager cm = (ConnectivityManager) FlickrAutoBackup.getAppContext().getSystemService(Context.CONNECTIVITY_SERVICE);
